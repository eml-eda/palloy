#!/usr/bin/env python3
"""
L1 size comparison test: Compare two different L1 sizes
Default: 64 KB vs 256 KB
Usage: python test_l1.py [size1_kb] [size2_kb]
Warning: Ensure that the workload fits in the specified L1 sizes (usually, at least 16 KB)
"""
...
def main():
    ...
    l1_size1 = int(sys.argv[1]) if len(sys.argv) > 1 else 64
    l1_size2 = int(sys.argv[2]) if len(sys.argv) > 2 else 256
    ...
    print(f"\n=== L1 Size Comparison: {l1_size1} KB vs {l1_size2} KB ===\n")
    
    # First configuration
    print(f"\n--- Running with L1 size {l1_size1} KB ---")
    ...
    result_file1 = out_dir / f"results_l1_{l1_size1}kb.json"
    sim1.save_results(str(result_file1))
    
    # Second configuration
    print(f"\n--- Running with L1 size {l1_size2} KB ---")
    ...
    result_file2 = out_dir / f"results_l1_{l1_size2}kb.json"
    sim2.save_results(str(result_file2))
    
    # Compare results
    print(f"\n=== Comparison Results ===")
    print(f"{'Metric':<20} {l1_size1:>10} KB {l1_size2:>10} KB {'Ratio':>12}")
    print("-" * 60)
